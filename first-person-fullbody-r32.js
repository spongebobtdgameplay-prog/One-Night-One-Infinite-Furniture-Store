...
const SHOULDER_BELOW_EYE_IDLE = 0.30;
const SHOULDER_BELOW_EYE_MOVING = 0.32;
const SHOULDER_BEHIND_EYE = 0.075;
...
const FIRST_PERSON_LOOK_DOWN = THREE.MathUtils.degToRad(58);
const FIRST_PERSON_LOOK_UP = THREE.MathUtils.degToRad(75);
...
const State = {
  ...
  ShoulderLeftPosition: new THREE.Vector3(),
  ShoulderRightPosition: new THREE.Vector3(),
  ShoulderMidpoint: new THREE.Vector3(),
  ShoulderTarget: new THREE.Vector3(),
  Translation: new THREE.Vector3(),
  HorizontalTarget: new THREE.Vector3(),
...
function ApplyFirstPersonLookLimit(Camera) {
  const Controls = window.__STORE_POINTER_CONTROLS__ || null;
  if (Controls) {
    Controls.minPolarAngle = Math.PI * 0.5 - FIRST_PERSON_LOOK_UP;
    Controls.maxPolarAngle = Math.PI * 0.5 + FIRST_PERSON_LOOK_DOWN;
  }

  State.CameraEuler.setFromQuaternion(Camera.quaternion, "YXZ");
  State.CameraEuler.x = THREE.MathUtils.clamp(State.CameraEuler.x, -FIRST_PERSON_LOOK_DOWN, FIRST_PERSON_LOOK_UP);
  State.CameraEuler.z = 0;
  Camera.quaternion.setFromEuler(State.CameraEuler);
  Camera.updateMatrixWorld(true);
}

function RestoreThirdPersonLookRange() {
  const Controls = window.__STORE_POINTER_CONTROLS__ || null;
  if (!Controls) return;
...
function AlignBodyToCamera(Pivot, Camera) {
  const ShoulderLeft = Pivot.getObjectByName("Shoulder.L");
  const ShoulderRight = Pivot.getObjectByName("Shoulder.R");
  if (!ShoulderLeft?.isBone || !ShoulderRight?.isBone) return false;

  State.SavedPivotPosition.copy(Pivot.position);
  State.SavedPivotQuaternion.copy(Pivot.quaternion);

  const Yaw = Math.atan2(State.HorizontalForward.x, State.HorizontalForward.z);
  Pivot.quaternion.setFromAxisAngle(WORLD_UP, Yaw);
  Pivot.updateMatrixWorld(true);

  ShoulderLeft.getWorldPosition(State.ShoulderLeftPosition);
  ShoulderRight.getWorldPosition(State.ShoulderRightPosition);
  State.ShoulderMidpoint.copy(State.ShoulderLeftPosition).add(State.ShoulderRightPosition).multiplyScalar(0.5);

  const ShoulderBelow = THREE.MathUtils.lerp(SHOULDER_BELOW_EYE_IDLE, SHOULDER_BELOW_EYE_MOVING, State.MoveBlend);
  State.ShoulderTarget.copy(Camera.position)
    .addScaledVector(State.HorizontalForward, -SHOULDER_BEHIND_EYE);
  State.ShoulderTarget.y = Camera.position.y - ShoulderBelow;

  State.Translation.copy(State.ShoulderTarget).sub(State.ShoulderMidpoint);

  State.Start.set(Pivot.position.x, BODY_SAMPLE_HEIGHT, Pivot.position.z);
  State.HorizontalTarget.copy(State.Start);
  State.HorizontalTarget.x += State.Translation.x;
  State.HorizontalTarget.z += State.Translation.z;
  ClampSegmentToStructures(State.Start, State.HorizontalTarget, BODY_WALL_RADIUS, BODY_WALL_GAP);

  Pivot.position.x += State.HorizontalTarget.x - State.Start.x;
  Pivot.position.z += State.HorizontalTarget.z - State.Start.z;
  Pivot.position.y += State.Translation.y;
  Pivot.updateMatrixWorld(true);
  return true;
}
...
function SolveArm(Pivot, Camera, Side, Phase) {
  ...
  const WristSide = Side * THREE.MathUtils.lerp(0.245, 0.185, Move);
  const ElbowSide = Side * THREE.MathUtils.lerp(0.315, 0.255, Move);
  const WristForward = THREE.MathUtils.lerp(0.43, 0.53, Move) + Swing * 0.065;
  const ElbowForward = THREE.MathUtils.lerp(0.235, 0.305, Move) + Swing * 0.028;
  const WristDown = THREE.MathUtils.lerp(0.22, 0.145, Move);
  const ElbowDown = THREE.MathUtils.lerp(0.17, 0.115, Move);
  ...
  const ProxyRenderer = {
    render(RenderScene, RenderCamera) {
      if (BasePlayer.IsThirdPerson?.()) {
        RestoreThirdPersonLookRange();
        Renderer.render(RenderScene, RenderCamera);
        return;
      }

      ApplyFirstPersonLookLimit(RenderCamera);
      ...
      if (!AlignBodyToCamera(Pivot, RenderCamera)) {
        Renderer.render(RenderScene, RenderCamera);
        return;
      }
      ...
      try {
        ...
        Pivot.quaternion.copy(State.SavedPivotQuaternion);
        Pivot.updateMatrixWorld(true);
      }
    }
  };
...
window.__STORE_FIRST_PERSON_FULLBODY_BUILD__ = "V0.11-R32";
