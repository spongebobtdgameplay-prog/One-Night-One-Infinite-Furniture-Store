...
Style.textContent = `
  ...
  body.StoreNeedsMouseCapture #MouseCaptureHint { opacity: 1; }
`;
document.head.appendChild(Style);

const Hint = document.createElement("div");
Hint.id = "MouseCaptureHint";
...
function Sensitivity() {
  ...
  return Number.isFinite(Value) ? THREEClamp(Value, 0.35, 2) : 0.92;
}
...
function UpdateCaptureState() {
  ...
  if (CurrentControls) {
    if (FirstPerson) CurrentControls.isLocked = Locked;
...
addEventListener("mousedown", Event => {
  if (Event.button !== 0 || !HudActive() || !IsFirstPerson() || NativePointerLocked()) return;
  const CurrentControls = Controls();
...
window.__STORE_INPUT_CONTROLS_BUILD__ = "V0.12.10";
