...
function CharacterYawFromCamera() {
...
function SetMode(ThirdPerson, RequestPointerLock = false) {
  ...
  if (State.ThirdPerson) {
    ...
    if (!WasThirdPerson) State.SyncFacingOnThird = true;
    if (document.pointerLockElement) {
      try { document.exitPointerLock(); } catch {}
    }
  } else {
    State.Distance = 0;
    if (State.Pivot) State.Pivot.rotation.y = CharacterYawFromCamera();
    const Controls = State.Controls || window.__STORE_POINTER_CONTROLS__ || null;
...
function CaptureEyeAnchor() {
  if (State.EyeAnchorReady || !State.Pivot || !State.Head?.isBone) return;
...
function RenderThirdPerson(Renderer, Scene, Camera) {
  ForceBodyVisible();
  PlaceCharacterAtLogicalPlayer(false);

  if (State.Pivot && State.SyncFacingOnThird) {
    State.Pivot.rotation.y = CharacterYawFromCamera();
    State.Pivot.updateMatrixWorld(true);
    State.SyncFacingOnThird = false;
  }
...
function RenderFirstPerson(Renderer, Scene, Camera) {
  ForceBodyVisible();
  PlaceCharacterAtLogicalPlayer(true);
  CaptureEyeAnchor();
  SavePose();
  ApplyFirstPersonPose();

  State.SavedCameraPosition.copy(Camera.position);
  if (State.Pivot && State.EyeAnchorReady) {
    State.TempEye.copy(State.EyeAnchorLocal);
    State.Pivot.localToWorld(State.TempEye);
    Camera.position.copy(State.TempEye);
    Camera.updateMatrixWorld(true);
  }
...
window.__STORE_PLAYER_SYSTEM_BUILD__ = "V0.11-R19";
