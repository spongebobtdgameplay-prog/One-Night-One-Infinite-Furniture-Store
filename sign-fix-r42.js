...
if (!Game?.Scene) throw new Error("Game must load before R42 sign fix.");
...
function LabelMaterial(Texture) {
  return new THREE.MeshStandardMaterial({ map: Texture, color: 0xffffff, roughness: 0.92, metalness: 0, emissive: 0x24170d, emissiveIntensity: 0.05, side: THREE.FrontSide });
}
...
function Upgrade(Group) {
  ...
  const Front = new THREE.Mesh(new THREE.PlaneGeometry(4.60, 1.02), LabelMaterial(Texture));
  Front.name = "SectionSignLabelFront";
  Front.position.z = 0.122;
  const Back = new THREE.Mesh(new THREE.PlaneGeometry(4.60, 1.02), LabelMaterial(Texture));
...
function Process() {
  for (const Chunk of Game.ActiveChunks?.values?.() || []) {
    Chunk.Group?.traverse?.(Object => {
      if (Object.name === "SectionSign") Upgrade(Object);
    });
  }
}

setInterval(Process, 650);
setTimeout(Process, 0);
window.__STORE_SIGN_FIX_BUILD__ = "V0.11-R42";
