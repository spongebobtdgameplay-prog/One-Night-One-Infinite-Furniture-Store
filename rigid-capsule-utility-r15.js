...
function CapsuleTouchesEntry(Capsule, Entry, Options = {}) {
  ...
  return BaseCollision.SegmentExpandedBoundsHit(Capsule.Start, Capsule.End, Bounds, Radius) !== null;
}
...
function TranslatedCapsuleSetBlocked(Capsules, Translation, Entries, Options = {}) {
  for (const Capsule of Capsules || []) {
    ...
    const Candidate = {
      Start: Scratch.CandidateStart,
      End: Scratch.CandidateEnd,
...
function SweepCapsuleSetFraction(Capsules, Motion, Entries, Options = {}) {
  ...
  const MaxSweepSteps = Math.max(4, Number(Options.MaxSweepSteps) || 40);
  const BinarySteps = Math.max(5, Number(Options.BinarySteps) || 13);
  const StepLength = Math.max(0.012, Number(Options.StepLength) || 0.028);
  const StepCount = THREE.MathUtils.clamp(Math.ceil(Length / StepLength), 1, MaxSweepSteps);
  let LastSafe = 0;

  for (let Step = 1; Step <= StepCount; Step += 1) {
    const Fraction = Step / StepCount;
    Scratch.Translation.copy(Motion).multiplyScalar(Fraction);
    if (!TranslatedCapsuleSetBlocked(Capsules, Scratch.Translation, Entries, Options)) {
      LastSafe = Fraction;
      continue;
    }
    ...
    for (let Binary = 0; Binary < BinarySteps; Binary += 1) {
      ...
      Scratch.Translation.copy(Motion).multiplyScalar(Mid);
      if (TranslatedCapsuleSetBlocked(Capsules, Scratch.Translation, Entries, Options)) High = Mid;
      else Low = Mid;
    }
    return Low;
  }
...
function ResolveCapsuleSetTranslation(StartPosition, Desired, Capsules, Entries, Options = {}) {
  Scratch.Motion.copy(Desired);
  Scratch.Motion.y = 0;
  const Length = Scratch.Motion.length();
  ...
  const Fraction = SweepCapsuleSetFraction(Capsules, Scratch.Motion, Entries, Options);
  const Skin = Math.max(0, Number(Options.Skin) || 0.005);
  const SafeFraction = Fraction < 0.999999
    ? Math.max(0, Fraction - Skin / Length)
    : 1;
  ...
  Scratch.Translation.copy(Scratch.Motion).multiplyScalar(Math.min(1, Fraction + 0.002));
  const Contact = FindTranslatedCapsuleSetContact(Capsules, Scratch.Translation, Entries, Options);
  return {
    Position,
    Resolved,
...
function ResolveHybridTranslation(StartPosition, Desired, Capsules, CoreRadius, Entries, Options = {}) {
  ...
  const FurnitureResult = BaseCollision.ResolveHorizontalMove(
    StartPosition,
    Desired,
    Math.max(0.01, Number(CoreRadius) || 0.22),
    Entries,
    {
      Skin: Math.max(0.002, Number(Options.CoreSkin) || 0.005),
      MaxIterations: 1,
      MaxSweepSteps: Math.max(20, Number(Options.MaxSweepSteps) || 40),
      BinarySteps: Math.max(8, Number(Options.BinarySteps) || 13),
      AllowSlide: false,
      Filter: NonStructureFilter
    }
  );

  if (!FurnitureResult.Hit) return StructureResult;
  if (!StructureResult.Hit) {
    return {
      Position: FurnitureResult.Position,
      Resolved: FurnitureResult.Resolved,
      Fraction: Desired.lengthSq() > 0.000001
        ? FurnitureResult.Resolved.length() / Desired.length()
        : 0,
      Hit: true,
  ...
  if (FurnitureResult.Resolved.lengthSq() >= StructureResult.Resolved.lengthSq()) return StructureResult;
  return {
    Position: FurnitureResult.Position,
    Resolved: FurnitureResult.Resolved,
    Fraction: Desired.lengthSq() > 0.000001
...
window.__STORE_RIGID_CAPSULE_UTILITY_BUILD__ = "V0.12.14-R15";
