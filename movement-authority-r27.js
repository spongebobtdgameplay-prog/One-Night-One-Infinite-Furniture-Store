...
if (!Game?.Camera || !Game?.CollisionBoxes || !Player || !Collision) {
  throw new Error("Game, player, and collision utility must load before validated movement authority.");
}

const CoreRadius = 0.285;
const ContactGap = 0.012;
const DominanceMargin = 0.06;
const DominanceBlendWidth = 0.22;
const TangentDeadzone = 0.08;
...
PointerLockControls.prototype.moveForward = function MoveForwardValidatedAuthority(Distance) {
...
PointerLockControls.prototype.moveRight = function MoveRightValidatedAuthority(Distance) {
  const Camera = ControlCamera(this);
...
window.__STORE_MOVEMENT_AUTHORITY_BUILD__ = "V0.12.29";
