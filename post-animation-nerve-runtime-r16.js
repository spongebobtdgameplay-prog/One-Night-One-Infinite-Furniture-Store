...
if (!Game?.Scene || !Game?.Camera || !Game?.CollisionBoxes || !Player || !Collision) {
  throw new Error("Game, player, and collision utility must load before post-animation nerves.");
}
...
const NerveSegments = [
  ...
  { Joint: "LowerArm.L", Child: "Wrist.L", Radius: 0.064, Preference: "down-left" },
  ...
  { Joint: "LowerArm.R", Child: "Wrist.R", Radius: 0.064, Preference: "down-right" },
...
PointerLockControls.prototype.moveForward = function MoveForwardWithStableCore(Distance) {
...
PointerLockControls.prototype.moveRight = function MoveRightWithStableCore(Distance) {
  const Camera = ControlCamera(this);
...
function PreferredDirection(Segment, Camera) {
  ...
  if (Segment.Preference.startsWith("down")) {
    PoseScratch.PreferredDirection.copy(PoseScratch.Down)
      .addScaledVector(PoseScratch.CameraRight, Side * 0.28)
      .addScaledVector(PoseScratch.CameraForward, -0.10)
      .normalize();
  } else {
    PoseScratch.PreferredDirection.copy(PoseScratch.CameraForward).multiplyScalar(-0.68)
      .addScaledVector(PoseScratch.CameraRight, Side * 0.20)
...
function ConstrainSegment(Pivot, Segment, Camera) {
  const Joint = Pivot.getObjectByName(Segment.Joint);
  const Child = Pivot.getObjectByName(Segment.Child);
  ...
  const Changed = RotateJointExactly(Joint, Child, PoseScratch.SafeEnd);
...
function ApplyFinalPoseNerves(Pivot, Camera) {
  for (let Pass = 0; Pass < 4; Pass += 1) {
    let Changed = false;
    for (const Segment of NerveSegments) {
      if (ConstrainSegment(Pivot, Segment, Camera)) Changed = true;
    }
    if (!Changed) break;
  }
}
...
if (typeof PreviousRender !== "function") throw new Error("Player render function is unavailable for post-animation nerves.");

Player.Render = function RenderWithFinalPoseNerves(Renderer, Scene, Camera) {
  const ProxyRenderer = {
    render(RenderScene, RenderCamera) {
      ...
      SavePose(Pivot);
      try {
        ApplyFinalPoseNerves(Pivot, RenderCamera);
        Renderer.render(RenderScene, RenderCamera);
      } finally {
        RestorePose(Pivot);
      }
...
window.__STORE_PART_NERVE_COLLISION_BUILD__ = "V0.12.15";
