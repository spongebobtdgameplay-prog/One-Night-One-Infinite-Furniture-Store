...
if (!Game?.Camera || !Game?.Scene || !Game?.CollisionBoxes || !Player || !Collision) {
  throw new Error("Game, player, scene, and collision utility must load before intent-gated facing slide authority.");
}
...
const Scratch = {
  ...
  IntentTangent: new THREE.Vector3(),
...
function NoSlide(IntentInward = 0, IntentTangent = 0, FacingTangent = 0) {
...
function SlideFromIntentAndFacing(Normal, IntentDirection) {
  const IntentNormal = IntentDirection.dot(Normal);
  const IntentInward = Math.max(0, -IntentNormal);

  Scratch.IntentTangent.copy(IntentDirection).addScaledVector(Normal, -IntentNormal);
  ...
  if (Dominance <= 0 || IntentTangentAmount <= 0.000001) {
    ...
    return NoSlide(IntentInward, IntentTangentAmount, 0);
  }

  Scratch.IntentTangent.normalize();
  const Facing = ReadCharacterFacing(IntentDirection);
  const FacingNormal = Facing.dot(Normal);
  Scratch.FacingTangent.copy(Facing).addScaledVector(Normal, -FacingNormal);
  Scratch.FacingTangent.y = 0;
  const FacingTangentAmount = Scratch.FacingTangent.length();

  if (FacingTangentAmount <= FacingDirectionDeadzone) {
  ...
  const Gate = Smooth01(Dominance / IntentBlendWidth);
  const Ratio = THREE.MathUtils.clamp(
    Math.min(IntentTangentAmount, FacingTangentAmount) * Gate,
    0,
    1
  );

  if (Ratio <= 0.000001) return NoSlide(IntentInward, IntentTangentAmount, FacingTangentAmount);

  return {
    Direction: Scratch.FacingTangent,
    Ratio,
    IntentInward,
    IntentTangent: IntentTangentAmount,
    FacingTangent: FacingTangentAmount
  };
}
...
function RecordContact(Result, Desired, Slide) {
  ...
  Contact.IntentInward = Slide.IntentInward;
  Contact.IntentTangent = Slide.IntentTangent;
  Contact.FacingTangent = Slide.FacingTangent;
  Contact.FacingAngle = THREE.MathUtils.radToDeg(Math.atan2(Slide.IntentTangent, Math.max(Slide.IntentInward, 0.000001)));
  Contact.Strength = THREE.MathUtils.clamp(0.58 + Slide.IntentInward * 0.42, 0, 1);
...
function ResolveMovement(Camera, ForwardAmount, RightAmount, Distance) {
  ...
  const Slide = SlideFromIntentAndFacing(Normal, Scratch.DesiredDirection);
...
PointerLockControls.prototype.moveForward = function MoveForwardIntentGatedFacingSlide(Distance) {
...
PointerLockControls.prototype.moveRight = function MoveRightIntentGatedFacingSlide(Distance) {
  const Camera = ControlCamera(this);
