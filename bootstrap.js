...
if (AccountGatePromise) {
  if (BootStatus) BootStatus.textContent = "Waiting for account selection...";
  window.__STORE_ACCOUNT_GATE_RESULT__ = await AccountGatePromise;
}

const Cache = "20260825-110";
...
window.__STORE_RUNTIME_LOADED__ = false;
...
function ShowBootError(Error) {
  ...
  if (BootStatus) BootStatus.textContent = "Store failed to load.";
}
...
function SetMenuReady() {
  ...
  if (BootStatus) BootStatus.textContent = "Menu ready — store engine asleep until you enter.";
}

// Pregame is intentionally tiny: account/session + one WebSocket + DOM UI only.
// No Three.js, showroom, lights, chunks, collision, NPCs or animation loops exist yet.
try {
  await import(`./multiplayer-lobby-client-r99.js?v=${Cache}`);
  await OptionalImport("./session-outdated-r93.js", "Stale-session refresh guard");
  await import(`./multiplayer-ui-r98.js?v=${Cache}`);
  window.__STORE_MULTIPLAYER_UI_R98__?.Render?.();
  SetMenuReady();
} catch (Error) {
  console.error("Pregame network/menu failed.", Error);
  ShowBootError(Error);
}

let RuntimePromise = null;
let RuntimeLoaded = false;
let EnterAfterLoad = false;
  ...
  RuntimePromise = (async () => {
    try {
      if (StartButton) {
        ...
        StartButton.textContent = "LOADING STORE...";
      }
      if (BootStatus) BootStatus.textContent = "Loading the store now — gameplay systems are starting...";
      ...
      await OptionalImport("./ui-performance-r95.js", "Hard-idle UI authority");
      await OptionalImport("./runtime-performance-buffer-r94.js", "Incremental distance and frame performance buffer");
      await OptionalImport("./multiplayer-gameplay-r99.js", "Multiplayer avatars and movement gameplay");
      await OptionalImport("./player-nameplate-r94.js", "Compact account player nameplates");
      await OptionalImport("./multiplayer-authority-r98.js", "Event-driven multiplayer task and clock authority");
      ...
      RuntimeLoaded = true;
      window.__STORE_RUNTIME_LOADED__ = true;
      if (Canvas) Canvas.style.visibility = "visible";
      return true;
    } catch (Error) {
      console.error("Core store runtime failed.", Error);
      ShowBootError(Error);
      return false;
    } finally {
      RuntimePromise = null;
...
async function LoadAndEnter() {
  if (EnterAfterLoad) return;
  EnterAfterLoad = true;
  const Loaded = await LoadRuntime();
  if (!Loaded) {
    EnterAfterLoad = false;
    return;
  }

  StartButton?.removeEventListener("click", CaptureFirstEnter, true);
  if (StartButton) {
    StartButton.disabled = false;
    StartButton.textContent = "ENTER THE STORE";
  }
...
function CaptureFirstEnter(Event) {
...
StartButton?.addEventListener("click", CaptureFirstEnter, true);
addEventListener("store-multiplayer-start", () => {
  if (!RuntimeLoaded) void LoadAndEnter();
});
