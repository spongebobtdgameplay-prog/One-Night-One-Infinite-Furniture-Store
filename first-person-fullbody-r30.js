...
const BODY_BACK_IDLE = 0.065;
const BODY_BACK_MOVING = 0.045;
const BODY_DOWN_IDLE = 0.045;
const BODY_DOWN_MOVING = 0.065;
...
function ApplyCenteredBodyOffset(Pivot) {
  ...
  Pivot.position.y -= THREE.MathUtils.lerp(BODY_DOWN_IDLE, BODY_DOWN_MOVING, State.MoveBlend);
  Pivot.updateMatrixWorld(true);
}
...
function SolveArm(Pivot, Camera, Side, Phase) {
  ...
  const WristSide = Side * THREE.MathUtils.lerp(0.24, 0.18, Move);
  const ElbowSide = Side * THREE.MathUtils.lerp(0.31, 0.25, Move);
  const WristForward = THREE.MathUtils.lerp(0.40, 0.50, Move) + Swing * 0.065;
  const ElbowForward = THREE.MathUtils.lerp(0.22, 0.29, Move) + Swing * 0.028;
  const WristDown = THREE.MathUtils.lerp(0.21, 0.14, Move);
  const ElbowDown = THREE.MathUtils.lerp(0.16, 0.11, Move);
...
window.__STORE_FIRST_PERSON_FULLBODY_BUILD__ = "V0.11-R30";
